...
mu_0 <- c(2, -1)
Sigma <- matrix(c(2/3, 0, 0, 2/3), 2)
X <- matrix(c(2.91, -1.84, 2.85, -1.10, 1.90, 0.32, 1.51, -1.37, 1.73, -1.50, 2.83, -0.42, 2.03, -1.20, 1.97, -0.60, 2.14, -1.27, 1.95, 0.54), nrow = 10, byrow = TRUE)
n <- nrow(X)
x_m <- colMeans(X)
n * t(x_m - mu_0) %*% solve(Sigma) %*% (x_m - mu_0)
