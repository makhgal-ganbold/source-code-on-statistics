...
# write.csv(x = as.data.frame(bikers), file = "bikers_2017.csv", row.names = FALSE)
...
bikers <- read.csv(file = "bikers_2017.csv")
user_ids <- bikers$id
...
rides$date <- as.POSIXct(paste(2017, rides$date), tz = "Asia/Ulaanbaatar", format = "%Y %m сар %d")

# write.csv(x = rides, file = "rides_2017.csv", row.names = FALSE)
# Үүний дараа тус өгөгдөлд "ride_id" гэсэн багана нэмэв.
# Тус баганын утгуудыг өөр эх сурвалжаас олсон тул write.csv() функцийг ахин ажиллуулах хэрэггүй.
# Харин цааш үргэлжлүүлэхдээ дараах мөрд буй read.csv() функцийг ажиллуулж тийнхүү өөрчилсөн өгөгдлийг ачаална.

rides <- read.csv(file = "rides_2017.csv")
...
write.csv(x = ride_user, file = "ride_user_2017.csv", row.names = FALSE)
